var SK = class {
    ...
    hook1(...args) {
      ...
      function setSpeed(speedup) {
          if (sk._autoTick) { clearInterval(sk._autoTick) }
          const tix = 1000 / speedup
          game.rate = 5 + (1000 / tix)
          console.log('Setting interval to', tix, 'rate', game.rate, sk._autoTick)
          sk._autoTick = setInterval(() => { game.tick() }, tix)
      }

      function coolit(heat = 10) {
          game.time.heat = heat
      }

      function don(times, func) {
          const res = []
      ...
      function mapValues(obj, func) {
          const res = {}
          Object.entries(obj).forEach(([kk, vv]) => { res[kk] = func(vv, kk) })
          return res
      }

      function maxem() {
          ['catnip', 'wood', 'minerals', 'coal', 'iron', 'titanium', 'gold', 'oil', 'uranium',
          'unobtainium', 'antimatter', 'manpower', 'science',  'culture', 'faith'].forEach((res) => {
              setres(res)
          })
      }
      ...
      function sked1(frac) {
          return {
            Field: 200,
            Hut: 10, Log: 80, Mansion: 30, Academy: 20, Workshop: 80, Mine: 80, Smelter: 80,
            Observ: 80, Trade: 80, Mint: 30, Temple: 80, Factory: 30, Reactor: 30,
            Oil: 50, Quarry: 50, Calciner: 50, Steam: 30, Zigg: 40, Magnet: 50, Bio: 80,
            Accel: 10, Satell: 30, Lunar: 20, Planet: 20, Hydrau: 20, Research: 5, Cryo: 10,
            Solar: 20, Hydro: 10
          }
      }
      ...
      function checklist() {
          return {
              leader: 'merchant',
              outo:  'bcoin, trade, praise',
              tab: 'bonfire',
              check: 'uncraftables, steel fraction, trade toggles, plate toggle',
              //
              export: 'do it',
              temporalFlux: 'bought?',
              switchOnSpaceStations: 'no forget kitty!!!',
              //
              buy: 'chronosphere, tankers, chronochamber THEN fix others, chronotheo buildings',
              alicorns: 'sacForTC',
              sellandtrade: 'elevator, array',
              sell: 'beacon, entanglement, ziggurat, factory, steamworks, quarry, calciner, ',
              maybesell: '?? dataCenter, biolab',
              shutdown: 'calciner, outposts, oilwellpower, biolab power, ',
              energyOK: 'magneto, steamworks, sunlifter, harvester, ',
              storage: 'oilWell, accelerator, cryostation, sunforge',
              sellandtradeLast: 'moonbase',
              bls: 'maxed',
              relicsOrBcoin: 'relicsmaybe?',
              //
              export: 'do it',
              temporalFlux: 'bought?',
              switchOnSpaceStations: 'no forget kitty!!!',
              //
              transcend: 'trans adore no praise',
              paragon: 'maybe',
          }
      }

      function buildAsc(max = 1) {
          const sked = sked1()
          const frac = max / 10
          console.log('Building fraction', frac)
          Object.entries(sked).forEach(([bldName, num]) => {
            console.log('Building fraction', bldName, frac, num)
            bldUpto(bldName, frac * num)
          })
      }

      function forceEmbassy() {
          setres('culture'); sk.tasks.autoEmbassy(10); sk.tasks.autoEmbassy(10); sk.tasks.autoEmbassy(10)
      }

      function nunicorns(goal = 10000) {
        const nzigs = bldCount('ziggurat');
        const nukes = resCount('unicorns');
        const ntear = resCount('tears');
        const ups   = resPerSec('unicorns');
        const tearPerSac = 2500;
        console.log(bldInfo('ziggurat'), resInfo('unicorns'), resInfo('tears'), tearPerSac);
        //
        const sacTear = nzigs * (nukes / tearPerSac);
        const totUkes = (goal / nzigs) * 2500;
        const remUkes = (((goal - ntear) / nzigs) * 2500) - nukes;
        console.log('Current:', nukes, '*', nzigs, '/', tearPerSac, '=', sacTear, 'total', sacTear + ntear);
        console.log('For', goal, 'tears need', totUkes, 'total', remUkes, 'more', remUkes / ups, 'secs');
        return remUkes;
      }

      function resInfo(rn)        { return game.resPool.get(rn); }
      function resCount(rn)       { return resInfo(rn).value; }
      function resNextCount(rn)   { return resCount(rn) + resPerTick(rn); }
      function resPerTick(rn)     { return game.getResourcePerTick(rn); }
      function resPerSec(rn, spt = 5) {
        return game.getResourcePerTick(rn) * spt;
      }

      function bldInfo(bname)     { return game.bld.get(bname); }
      function bldCount(bname)    { return bldInfo(bname).val; }

      function curSeason()        { return game.calendar.getCurSeason(); }
      function curSeasonName()    { return curSeason().name; }
      function tradeRatio()       { return game.diplomacy.getTradeRatio() }

      const goldPerTrade = 15;
      function civInfo(civname) { return game.diplomacy.get(civname); }
      function civCanTrade(civname) { return civInfo(civname).unlocked; }
      function civGivesInfo(civname) { return civInfo(civname).sells; }
      function civPolicyBonus(civname) { return game.diplomacy.calculateTradeBonusFromPolicies(civname, game) }

      function civGivesCount(civname, sellCount = 1) {
        const telist = []
        for (const res of civGivesInfo(civname)) {
          const seasonAdj = 1 + (res.seasons ? res.seasons[curSeasonName()] : 0);
          const tradeAdj  = (1 + tradeRatio() + tradePolicyBonus(civname));
          telist.push(res.value * sellCount * seasonAdj * tradeAdj);
        }
        return telist;
      }
      ...
      function spaceBtns() {
          ...
          return game.spaceTab.planetPanels.reduce((acc, { children }, ii) => acc.concat(children), [])
      }

      function bldBtn(re) {
        const bld = game.bldTab.children.find(({ model: { name } ={}}) => RegExp(re).test(name))
        if (bld) { return bld }
        const spa = spaceBtns().find(({ model: { name } = {}}) => RegExp(re).test(name))
        if (spa) { return spa }
        return null
      }
      ...
      function bldBuy(re) {
        ...
        return new Promise((yay, boo) => {
          btn.controller.buyItem(btn.model, {}, (result) => {
            if (result) {
              game.msg('Built a ' + btn.model.metadata.name);
              btn.update();
              yay(btn)
            }
            yay({})
      ...
      function ntimes(max) {
          const ret = []
          for (let ii = 0; ii < max; ii += 1) { ret.push(ii) }
          return ret
      }
      ...
      function sleep(delay = 1000) { return new Promise((yay) => setTimeout(yay, delay)) }

      async function bldUpto(re, max) {
         for (const ii of ntimes(max)) {
            const btn = bldBtn(re)
            const model = btn?.model
            if (! model) { console.error(re, 'Not found'); return {} }
            if (model.on >= max) { console.log('Goal built:', model.name, model.on, max); return btn }
            // console.log('Building', model.name, model.on, 'up to', max)
            const res = await bldBuy(re)
            // console.log('result:', res)
            // if (! (res && res.model)) { continue }
            await sleep(200)
         }
         return bldBtn(re)
      }

      function makeTrade(civname, maxTrades, amount) {
        const civ = game.diplomacy.get('dragons');
        game.msg('Auto trading ' + amount + ' with ' + civname);
        game.diplomacy.tradeMultiple(civ, Math.min(maxTrades, amount));
      }

      function setres(rn, adj = 1) {
        if (! rn) { console.log('No resource given'); return };
        const res = resInfo(rn);
        const max = res.maxValue ? Math.max(res.value, res.maxValue) : (2 * res.value);
        const newval = (adj <= 1 ? adj * max : adj);
        res.value = newval;
      }
      ...
      return {
        nunicorns, resInfo, resCount, resNextCount, resPerTick, resPerSec,
        bldInfo, bldCount, checklist,
        curSeason, curSeasonName, tradeRatio,
        civInfo, civCanTrade, civGivesInfo, civGivesCount, civPolicyBonus,
        makeTrade,
        setres, maxem,
        whenslip,
        don, forceEmbassy, bldBtn, bldBuy, bldUpto,
        sleep, ntimes,
        buildAsc, mapValues, sked1, setSpeed, coolit,
      }
    }
    ...
    setres(name, adj = 1) {
      ...
      const max = res.maxValue || (2 * res.value);
      const newval = (adj <= 1 ? adj * max : adj);
      res.value = newval;
    }
...
SK.Tasks = class {
    constructor(model, scripts) {
        ...
        this.schedule = [
            ...
            {fn:'autoHunt',     interval:10,  offset:7,   override:false},
            ...
            {fn:'autoTrade',    interval:1, offset:0,  override:false},
    ...
    autoCraft(ticksPerCycle) {
        ...
        if (this.model.auto.craft && game.workshopTab.visible) {
            for (const res of game.workshop.crafts) {
                ...
                if (/eludium|thorium|tanker/.test(output)) { continue; }
                ...
                for (const input of inputs) {
                    ...
                    // pk4
                    const minIronToMakeSteelFrac = 0.3; const skipSteelAnywayFrac = 0.1;
                    if (output === 'steel' && input.name === 'iron' && craftCount > 0) {
                      const ironRes = game.resPool.get('iron');
                      if ((ironRes.value > minIronToMakeSteelFrac * ironRes.maxValue) && (Math.random() > skipSteelAnywayFrac)) {
                        continue;
                      }
                    };
                    ...
                    // if (output === 'steel' && craftCount > 0) { console.log(output, inVal, outVal, craftCount); }
    ...
    autoHunt(ticksPerCycle) {
        if (this.model.auto.hunt && (game.villageTab.visible || game.resPool.get('zebras').value > 0)) {
                ...
                if (program.model.metadata.unlocked && program.model.on == 0) {
                    ...
                    if (program.model.enabled) {
                        program.controller.buyItem(program.model, {}, function(result) {
                            if (result) {
                                game.msg('Launched a ' + program.model.name);
                    ...
                    for (const button of buttons) {
                        if (tb[button.id]?.enabled && button.model.metadata.unlocked
                            ...
                            if (button.model.enabled) {
                                button.controller.buyItem(button.model, {}, function(result) {
                                    if (result) {
                                        game.msg('Raptured up a ' + button.model.name);
                                        built = true; button.update();
                                    }
                                });
                            }
    ...
    autoTechHelper(buttons, skipUpdate) {
        ...
        if (bestButton) {
            bestButton.controller.buyItem(bestButton.model, {}, function(result) {
                if (result) {
                    game.msg('Teched a ' + bestButton.model.name);
        ...
        if (this.model.auto.religion && game.religionTab.visible) {
            ...
            for (const button of game.religionTab.rUpgradeButtons) {
                ...
                if (button.model.enabled) {
                    button.controller.buyItem(button.model, {}, function(result) {
                        if (result) {
                            game.msg('Faith brought us a ' + button.model.name);
    ...
    autoTrade(ticksPerCycle) {
        ...
        if (this.model.auto.trade) { // && game.diplomacyTab.visible) {
            ...
            let powerPerCycle = Math.max(50, game.getResourcePerTick('manpower') * ticksPerCycle);
            powerPerCycle = Math.min(powerPerCycle, powerResource.value); // don't try to spend more than we have
            let sellCount = Math.floor(Math.min(3 * goldPerCycle/14, 3 * powerPerCycle/50));
            ...
            let maxTrades = 0;
            if (goldResource.value > 0.85 * (goldResource.maxValue - goldPerCycle)) { // don't check catpower
            ...
            if (maxTrades > 0) {
                ...
                const mnRes = game.resPool.get('minerals');
                const slRes = game.resPool.get('slab');
                const unoRes = game.resPool.get('unobtainium');

              //               if ((irRes.value > 1e6) && (irRes.value > 0.4 * irRes.maxValue) && (Math.random() < 0.95)) {
              ...
              //               }
                ...
                const haveEnough = unoRes.value >= 5000 && (unoRes.value >= unoRes.maxValue * 0.2)
                if (haveEnough && game.diplomacy.get('leviathans').unlocked && this.model.minor.elderTrade) {
                    game.msg('Trading ' + Math.floor(unoRes.value / 5000) + ' with leviathans');
                    game.diplomacy.tradeAll(game.diplomacy.get('leviathans'));
                    traded = true;
                } else if (tiRes.value < (tiRes.maxValue * 0.9) && (slRes.value > 50) && game.diplomacy.get('zebras').unlocked) {
                    // game.craft('slab', 1);
                ...
                } else if (doGriffs && irRes.value < (irRes.maxValue * 0.99) && game.diplomacy.get('griffins').unlocked && Math.random() < 0.9) {
                    // pk4
                    game.msg('Force trading with griffins');
                    /// const amt = Math.min(maxTrades, 4000); // cap or probability adjusts how much we hurt mint
                    // game.diplomacy.tradeMultiple(game.diplomacy.get('griffins'), amt);
                    game.diplomacy.tradeAll(game.diplomacy.get('griffins'));
                } else if (doNagas && (mnRes.value < (mnRes.maxValue * 0.9)) && game.diplomacy.get('nagas').unlocked && Math.random() < 0) {
                     // pk4
                  // const amt = Math.min(maxTrades, 3);
                  // game.msg('Force trading ' + amt + ' with nagas');
                  // game.diplomacy.tradeMultiple(game.diplomacy.get('nagas'), amt);
                } else if (doDrags && (tiRes.value > (tiRes.maxValue * 0.9)) && game.diplomacy.get('dragons').unlocked && Math.random() < 0.5) {
                     // pk4
                     const amt = Math.min(maxTrades, 9000);
                     game.msg('Force trading ' + amt + ' with dragons');
                     game.diplomacy.tradeMultiple(game.diplomacy.get('dragons'), amt);
                }
    ...
    autoEmbassy(ticksPerCycle) {
        ...
        if (this.model.auto.embassy && game.science.get('writing').researched && game.diplomacyTab.racePanels && game.diplomacyTab.racePanels[0]) {
            ...
            // console.log(culture);
            if (culture.value >= culture.maxValue * 0.7) { // will often exceed due to MS fluctuations
                ...
                btn.controller.buyItem(btn.model, {}, function(result) {
                    if (result) {
                        const friends = btn.race && btn.race.name;
                        game.msg('Cemented embassy with ' + friends);
            ...
            if (bestButton != null) {
                ...
                if (otherCosts) {
                    ...
                    if (sufficient) {
                        ...
                        bestButton.controller.buyItem(bestButton.model, {}, function(result) {
                            if (result) {
                                game.msg('Sacrifice has brought about a ' + bestButton.model.name);
                                acted = true; bestButton.update();
                            }
                        });
                    }
...
SK.Scripts = class {
    ...
    slowloop(action) {
        switch (action) {
            ...
            case 'workshop-end': // -|
                var extraWorkshop = ['caravanserai', 'chronoforge', 'amBases', 'aiBases', 'distortion', 'turnSmoothly'];
        ...
        switch (action) {
            ...
            case 'build-start': // -> religion, steamworks, pop-max-cath
                ...
                for (const bname in this.model.cathBuildings) {
                    ...
                    case 'aqueduct': case 'pasture':
                            limit = 100;
                            break;
                        case 'warehouse': case 'observatory': case 'lumberMill': case 'field':
                            limit = 200;
                            break;
                        case 'chronosphere':
                            limit = 5;

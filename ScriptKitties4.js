    ...
    autoCraft(ticksPerCycle) {
        ...
        if (this.model.auto.craft && game.workshopTab.visible) {
            for (const res of game.workshop.crafts) {
                ...
                if (/megalith|gear|alloy|concrete|kerosene|eludium|thorium|tanker/.test(output)) { continue; }
                ...
                for (const input of inputs) {
                    ...
                    // pk4
                    if (output === 'steel'   && input.name === 'iron'  && craftCount > 0) { if (Math.random() < 0.95) { continue } };
